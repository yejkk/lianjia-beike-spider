...
class HLGBaseSpider(BaseSpider):
    ...
    def get_xiaoqu_info(city, area):
        ...
        for i in range(1, total_page + 1):
            ...
            for house_elem in house_elems:
                ...
                build_info = positionInfo.text.replace("\n", "").strip()
                result = pattern.findall(build_info)
                if len(result) >= 1:
                    build_info = result[0] + "建成"
                else:
                    build_info = "暂无"
                if price == None:
                    print(name)
                ...
                # 作为对象保存
                xiaoqu = XiaoQu(chinese_district, chinese_area, name, price, on_sale,build_info)
    ...
    def start(self):
        ...
        t1 = time.time()  # 开始计时
        self.collect_area_xiaoqu_data("bj","huilongguan2","csv")
